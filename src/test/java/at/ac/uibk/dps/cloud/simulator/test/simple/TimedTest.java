...
public class TimedTest extends TestFoundation {
	...
	public class SingleFire extends Timed {
		...
		public void tick(final long fires) {
			Assert.assertEquals("Timed event on the wrong time", expectedFire,
					fires);
			myfires++;
			doCancel();
		}
	...
	public void singleEventFire() {
		...
		Assert.assertEquals("Timed frequency is not the one expected",
				setFrequency, fire.getFrequency());
		Assert.assertEquals("Event distance is not correct", Long.MAX_VALUE,
				fire.nextEventDistance());
	...
	public void repeatedEventFire() {
		...
		Timed.simulateUntilLastEvent();
		Assert.assertEquals("Not enough timed events", expectedFires,
				fire.myfires);
	}
	...
	public void overLappingFires() {
		...
		Assert.assertEquals("Not enough timed events", expectedFires,
				fire.myfires);
		Assert.assertEquals("Overlapped timed event did not arrive",
				sf.get(0).myfires, 1);
	}
	...
	public void eventSkipper() {
		...
		Timed.simulateUntilLastEvent();
		Assert.assertEquals("Not the reduced amount of fires arrived",
				reducedFires, fire.myfires);
	}
	...
	public void eventJumper() {
		...
		Assert.assertEquals("Jump should have had success", 0, jump);
		Assert.assertEquals("Time after jump is not correct", expectedAfter,
				Timed.getFireCount());
	}
	...
	public void prematureStopper() {
		RepeatedFire fire = new RepeatedFire();
		Timed.simulateUntil(Timed.getFireCount() + (expectedFires - 3)
				* setFrequency);
		Assert.assertEquals("Not the reduced amount of fires arrived", 7,
				fire.myfires);
		Timed.simulateUntil(Long.MAX_VALUE);
		Assert.assertEquals("Not all events arrived", expectedFires,
				fire.myfires);
		Assert.assertTrue("The simulation finshed way into the future",
				Timed.getFireCount() == fire.maxTime + 1);
		SingleFire sf = new SingleFire();
		long expectedTime = Timed.getFireCount() + 2;
		Timed.simulateUntil(expectedTime);
		Assert.assertTrue("Fired events that should not arrive yet",
				sf.myfires == 0);
		Assert.assertEquals(
				"The simulation did not finish at the specified time",
	...
	public void multiSubscribe() {
		RepeatedFire fire = new RepeatedFire() {
			@Override
			public void tick(long fires) {
				Assert.assertFalse("Unexpected success of subscription",
						subscribe(3));
				super.tick(fires);
				if (maxTime == fires) {
					Assert.assertFalse("Unsubscribe should be unsuccessful",
							unsubscribe());
				}
			}
		};
	...
	public void unSubscribeWhileQueued() {
		...
		while (Timed.getFireCount() < fire.maxTime - reducedFires
				* setFrequency) {
			Timed.jumpTime(Long.MAX_VALUE);
			Timed.fire();
		}
		Assert.assertEquals("Incorrect number of timed events before cancel",
				reduction, fire.myfires);
		fire.cancel();
		Timed.simulateUntilLastEvent();
		Assert.assertEquals("Incorrect number of timed events after cancel",
				reduction, fire.myfires);
	}
	...
	public void freqUpdater() {
		...
		long adjustedFires = 1
				+ ((fire.maxTime - setFrequency) - Timed.getFireCount())
				/ changedFreq;
		Timed.simulateUntilLastEvent();
		Assert.assertEquals("Not enough timed events", adjustedFires,
				fire.myfires);
		...
		Timed.simulateUntilLastEvent();
		Assert.assertTrue(
				"Frequency change unsuccessful if not changed during the tick function",
				f.myfires == 1);
	}
